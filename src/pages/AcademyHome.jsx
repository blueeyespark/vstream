...
import { BookOpen, Hammer, Sparkles, Users, FolderKanban, ArrowRight, Compass, WandSparkles, Menu, Radio, LifeBuoy, GraduationCap } from "lucide-react";
import { useAuth } from "@/lib/AuthContext";
import { useBlue } from "@/lib/BlueContext";

const cards=[
  {title:"Continue Learning",detail:"Return to lessons, projects, feedback and mastery.",icon:BookOpen,to:"/Academy/Courses/3d-modeling-fundamentals"},
  {title:"Projects",detail:"Professional work, assignment versions and revisions stay organized here.",icon:FolderKanban,soon:true},
  {title:"Student Community",detail:"Clubs, study groups, campus events and Academy spaces in WorldChat.",icon:Users,to:"/WorldChat"},
  {title:"Creator Campus",detail:"Streaming, video, CreatorOS and professional creator pathways.",icon:Radio,to:"/Academy/CreatorCampus"},
];
...
export default function AcademyHome(){
  ...
  return <div className="min-h-[calc(100vh-4rem)] bg-[#03080f] text-white">
    ...
    <main className="mx-auto max-w-7xl px-5 py-10 sm:px-8">
      ...
      </div><div className="mt-5 flex flex-wrap gap-x-6 gap-y-2 border-y border-[#12305f]/50 py-4 text-xs font-bold text-blue-100/45"><Link to="/Academy/StudentCenter" className="hover:text-white"><GraduationCap className="mr-1 inline h-4 w-4"/> Student Center</Link><Link to="/Academy/Explore" className="hover:text-white"><LifeBuoy className="mr-1 inline h-4 w-4"/> School directory</Link><Link to="/CreatorOS" className="hover:text-white"><Hammer className="mr-1 inline h-4 w-4"/> CreatorOS workspace</Link><Link to="/" className="hover:text-white">VStream</Link></div>
