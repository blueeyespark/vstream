...
export default function AcademyHome(){
 const {user,isAuthenticated}=useAuth(); const blue=useBlue(); const [dashboard,setDashboard]=useState(null);
 useEffect(()=>{if(!isAuthenticated)return;let live=true;platform.academy.dashboard().then(x=>{if(live)setDashboard(x)}).catch(()=>{});return()=>{live=false}},[isAuthenticated]);
 const learningCards=useMemo(()=>{if(!dashboard?.enrollments?.length)return learning;const real=dashboard.enrollments.map(e=>({title:e.title,meta:e.summary||"Blue Academy course",progress:e.progress_percent||0,to:`/Academy/Courses/${e.slug}`,status:e.status==="completed"?"Completed":"Continue learning"}));return [...real,...learning.filter(p=>!real.some(r=>r.title===p.title))].slice(0,4)},[dashboard]);
 return <div className="min-h-screen bg-[#03080f] text-white">
  ...
  <main className="mx-auto max-w-[1450px] px-5 py-8 sm:px-8">
   <section><div className="mb-4 flex items-center justify-between"><div><p className="text-xs font-black uppercase tracking-[.2em] text-[#00c8ff]">Your learning journey</p><h2 className="mt-1 text-2xl font-black">Keep building your skills</h2></div><Link to="/Academy/Explore" className="text-xs font-black text-[#7ddcff]">View courses →</Link></div><div className="grid gap-4 md:grid-cols-3">{learningCards.map(x=><article key={x.title} className="rounded-2xl border border-[#12305f]/70 bg-[#06101f]/80 p-5"><div className="flex items-start justify-between gap-3"><BookOpen className="h-5 w-5 text-[#00c8ff]"/>{x.building&&<span className="rounded-full bg-amber-400/10 px-2 py-1 text-[10px] font-black text-amber-300">PLANNED</span>}</div><h3 className="mt-4 font-black">{x.title}</h3><p className="mt-1 text-xs text-blue-100/40">{x.meta}</p><div className="mt-5 h-1.5 overflow-hidden rounded-full bg-[#12305f]/60"><div className="h-full rounded-full bg-[#1e78ff]" style={{width:`${x.progress}%`}}/></div><div className="mt-2 flex justify-between text-[10px] font-bold text-blue-100/35"><span>{x.status}</span><span>{x.progress}%</span></div>{x.to?<Link to={x.to} className="mt-4 block rounded-lg border border-[#1e78ff]/40 py-2 text-center text-xs font-black text-[#7ddcff]">Continue</Link>:<button disabled className="mt-4 w-full rounded-lg border border-[#12305f] py-2 text-xs font-black text-white/25">Building</button>}</article>)}</div></section>
