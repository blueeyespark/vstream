...
import { Upload, Play, Pause, Download, Trash2, Volume2, Zap, Type, Palette, Music, Image, X } from "lucide-react";
...
export default function VideoEditorAdvanced() {
  ...
  const handleVideoUpload = (e) => {
    ...
    if (selectedFile) {
      if (selectedFile.size > 5 * 1024 * 1024 * 1024) { toast.error("File must be smaller than 5GB"); return; }
  ...
  const handleLoadedMetadata = () => {
    if (videoRef.current) { setDuration(videoRef.current.duration); setEndTime(videoRef.current.duration); }
  };
  ...
  const drawThumbnail = () => {
    ...
    if (uploadedImage) {
      const img = new Image();
      img.onload = () => { ctx.drawImage(img, 0, 0, preset.width, preset.height); drawText(); };
  ...
  useEffect(() => { drawThumbnail(); }, [bgColor, textColor, mainText, fontSize, uploadedImage, preset]);
    ...
    <div className="space-y-6">
      <Tabs value={tab} onValueChange={setTab}>
        <TabsList className="mb-6 flex flex-wrap gap-1 h-auto">
          <TabsTrigger value="video" className="gap-2"><Upload className="w-4 h-4" /> Video</TabsTrigger>
          <TabsTrigger value="thumbnail" className="gap-2"><Palette className="w-4 h-4" /> Thumbnail</TabsTrigger>
            ...
            {video ? (
              <div className="space-y-4">
                <div className="bg-black rounded-xl overflow-hidden aspect-video flex items-center justify-center" style={{ filter: `brightness(${brightness}%) contrast(${contrast}%)` }}>
                  <video ref={videoRef} src={video.url} onTimeUpdate={handleTimeUpdate} onLoadedMetadata={handleLoadedMetadata} className="w-full h-full object-contain" />
                </div>
                <div className="space-y-3">
                  <div className="flex justify-between text-sm text-slate-600 dark:text-blue-400/60">
                    <span>{currentTime.toFixed(2)}s</span><span>{duration.toFixed(2)}s</span>
                  </div>
                  <input type="range" min="0" max={duration} value={currentTime} onChange={(e) => { if (videoRef.current) videoRef.current.currentTime = parseFloat(e.target.value); setCurrentTime(parseFloat(e.target.value)); }} className="w-full" />
                </div>
                <div className="flex gap-2">
                  ...
                  <Button variant="outline" onClick={() => setVideo(null)} className="gap-2"><Trash2 className="w-4 h-4" /></Button>
                </div>
              </div>
            ) : (
          ...
          <div className="space-y-4">
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="bg-white dark:bg-[#060d18] rounded-2xl border border-slate-200 dark:border-blue-900/40 shadow-sm p-4">
              ...
              <div className="space-y-2">
                {presets.map((p) => (
                  <button key={p.name} onClick={() => setPreset(p)} className={`w-full p-2 text-sm rounded-lg border-2 transition-all ${preset.name === p.name ? "border-cyan-500 bg-cyan-50 dark:bg-cyan-900/20 font-medium text-cyan-900 dark:text-cyan-300" : "border-slate-200 dark:border-blue-900/40 text-slate-700 dark:text-blue-400/60 hover:border-slate-300"}`}>
                    {p.name} ({p.ratio})
                  </button>
                ))}
              </div>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.05 }} className="bg-white dark:bg-[#060d18] rounded-2xl border border-slate-200 dark:border-blue-900/40 shadow-sm p-4">
              <h3 className="font-semibold text-slate-900 dark:text-[#e8f4ff] mb-3 flex items-center gap-2"><Palette className="w-4 h-4" /> Colors</h3>
              <div className="space-y-3">
                <div>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }} className="bg-white dark:bg-[#060d18] rounded-2xl border border-slate-200 dark:border-blue-900/40 shadow-sm p-4">
              <h3 className="font-semibold text-slate-900 dark:text-[#e8f4ff] mb-3 flex items-center gap-2"><Type className="w-4 h-4" /> Text</h3>
              <div className="space-y-3">
                <div>
        ...
        <TabsContent value="intros" className="grid lg:grid-cols-3 gap-6">
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white dark:bg-[#060d18] rounded-2xl border border-slate-200 dark:border-blue-900/40 shadow-sm p-6">
              ...
              <div className="aspect-video rounded-xl flex items-center justify-center overflow-hidden" style={{ background: selectedTemplate.bg }}>
                <div className={`text-center ${getAnimationClass()}`}>
                  ...
                  <p style={{ fontSize: `${introFontSize}px`, color: selectedTemplate.textColor }} className="font-black">{introText}</p>
                </div>
              </div>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white dark:bg-[#060d18] rounded-2xl border border-slate-200 dark:border-blue-900/40 shadow-sm p-6">
              ...
              <div className="aspect-video rounded-xl flex items-center justify-center overflow-hidden" style={{ background: selectedTemplate.bg }}>
                <div className={`text-center ${getAnimationClass()}`}>
                  ...
                  <p style={{ fontSize: `${introFontSize}px`, color: selectedTemplate.textColor }} className="font-black">{outroText}</p>
                </div>
              </div>
          ...
          <div className="space-y-4">
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="bg-white dark:bg-[#060d18] rounded-2xl border border-slate-200 dark:border-blue-900/40 shadow-sm p-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                {introTemplates.map((t) => (
                  <button key={t.id} onClick={() => setSelectedTemplate(t)} className={`p-3 rounded-lg border-2 transition-all ${selectedTemplate.id === t.id ? "border-cyan-500 bg-cyan-50 dark:bg-cyan-900/20" : "border-slate-200 dark:border-blue-900/40 hover:border-slate-300"}`}>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }} className="bg-white dark:bg-[#060d18] rounded-2xl border border-slate-200 dark:border-blue-900/40 shadow-sm p-4">
              <h3 className="font-semibold text-slate-900 dark:text-[#e8f4ff] mb-4 flex items-center gap-2"><Type className="w-4 h-4" /> Text</h3>
              <div className="space-y-4">
                <div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white dark:bg-[#060d18] rounded-2xl border border-slate-200 dark:border-blue-900/40 shadow-sm p-6">
            ...
            {music ? (
              ...
              <label className="flex flex-col items-center justify-center aspect-video border-2 border-dashed border-slate-300 dark:border-blue-900/40 rounded-lg cursor-pointer hover:border-slate-400 transition-colors">
                ...
                <input type="file" accept="audio/*" onChange={(e) => { const file = e.target.files?.[0]; if (file) setMusic({ name: file.name, url: URL.createObjectURL(file), duration: 120 }); }} className="hidden" />
              </label>
            )}
          </motion.div>
        </TabsContent>
        ...
        <TabsContent value="media" className="space-y-4">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white dark:bg-[#060d18] rounded-2xl border border-slate-200 dark:border-blue-900/40 shadow-sm p-6">
            <div className="flex gap-2 mb-4">
              <Input placeholder="Search media..." value={searchMedia} onChange={(e) => setSearchMedia(e.target.value)} className="flex-1" />
            </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 max-h-[600px] overflow-y-auto">
                {filteredMedia.map(m => (
                  <motion.div key={m.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="group relative rounded-lg overflow-hidden bg-slate-100 dark:bg-[#050a14] border border-slate-200 dark:border-blue-900/40 hover:border-blue-500/50 transition-colors cursor-pointer">
                    {m.type === "image" && m.url && <img src={m.url} alt={m.name} className="w-full aspect-square object-cover group-hover:scale-105 transition-transform" />}
                    {m.type === "audio" && <div className="w-full aspect-square flex items-center justify-center"><Music className="w-12 h-12 text-blue-400/40" /></div>}
                    {m.type === "video" && <div className="w-full aspect-square flex items-center justify-center bg-black"><Play className="w-12 h-12 text-blue-400/40" fill="currentColor" /></div>}
